...
Each design task also produces its own [chunk scope](chunk-scope.md) — the bounded record of exactly which behaviors that task introduced, changed, or removed, tagged new/mutated/deleted — once the task itself is complete. This is what `Chunk The Design` (§3) actually consumes; a Feature's design directory may hold more design history than any one chunking pass needs.
### 3 Chunk The Design
The entry requirements of the `Chunk Design` step of the workflow are the [design directory and hld](design-directory-and-hld.md), the designs [specific behaviors](specific-behaviors.md), and the design task's own [chunk scope](chunk-scope.md) — the bounded list of behaviors, tagged new/mutated/deleted, that this specific design task actually produced. A Feature's design directory may hold more design history than any one chunking pass needs; chunk scope is what says exactly which part of it this pass covers.
...
The process of chunking the design breaks the design down into incrementally deliverable Chunks. Each Chunk **must** deliver at least 1 of the [specific behaviors](specific-behaviors.md) named in [chunk scope](chunk-scope.md) such that end-to-end (e2e) testing can verify that delivering the Chunk adds (or, for a `mutated`/`deleted` behavior, correctly changes) that specific behaviour in the project.
