...
`Architect Solution` decides the Service topology and data flow that will satisfy a Feature's use cases — naming,
for each in-scope use case, the functional boundary (a Service, the common case; a Library, specifiable but not
deployed, accommodated but not yet elaborated here — `concepts/service.md` §1) each of its operations is actually
specified against. It's Architecture's own responsibility paired with Analysis (Weaver Engineering Workflows §1),
distinct from `Architect Feature`'s own pairing with Analysis (which decides the Product Offering, not the
boundary) — not a Design activity, and not required before Design can start (§4 below).
...
**Why this step is `Architect Solution`, distinct from `Architect Feature`.** An earlier version of this process
used one name, `Architect Feature`, for both deciding the Service topology and deciding the Product Offering — two
genuinely different decisions that happened to be made by the same Architecture-paired-with-Analysis responsibility.
Naming the boundary-deciding half `Architect Solution` is what actually re-centers this process on a spec being
written against a defined functional boundary (Weaver Engineering Workflows, intro): the boundary is this step's
own, sole output, not one of two unrelated things a single step happens to produce.
